...
for(i in SRT)
{
  print(
    Sero_SRT %>% 
      ...
      geom_smooth(method = "lm", se = F))
  ggsave(paste0("MSimages/IgGvs", i, ".png"), width = 20, height = 15, units = "cm")
}
```
...
{ 
  print(PNT_SRT %>%
    filter(Marker1 == "CD4") %>%
    filter(!(Timepoint == "PI" | Timepoint == "VC")) %>% 
    filter(Marker2 == "CD40L+ CD137+" | Marker2 == "CD40L+ IL-21+" | Marker2 == "CD40L+ TNF-a+") %>% 
    filter(exclude_B.1.1.7 == 0) %>% 
    as.data.frame() %>%
    ggplot(., aes(x = B.1.1.7, y = !!sym(i))) +
      ...
      facet_wrap(~factor(Marker2, levels = c("CD40L+ CD137+",
                 ...
                 scales = "free") +
      theme(strip.background =element_rect(fill="aliceblue")) +
      ggtitle(paste("B.1.1.7 vs ", i)) +
      stat_cor(method = "spearman", size = 4))
  ggsave(paste0("MSimages/B.1.1.7vs", i, ".png"), width = 20, height = 15, units = "cm")
}
  ...
  print(PNT_SRT %>%
    ...
    ggplot(., aes(x = B.1.617.2, y = !!sym(i))) +
      ...
      facet_wrap(~factor(Marker2),#, levels = c("CD40L+ CD137+",
                 ...
                 scales = "free") +
      theme(strip.background =element_rect(fill="aliceblue")) +
      ggtitle(paste("B.1.617.2 vs ", i)) +
      stat_cor(method = "spearman", size = 4))
  ggsave(paste0("MSimages/B.1.617.2vs", i, ".png"), width = 20, height = 15, units = "cm")
}
