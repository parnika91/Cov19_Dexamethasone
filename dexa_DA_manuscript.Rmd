...
# FACS results
facs <- read_excel("../20220118 Mastertable FACS incl. dead and VC.xlsx") %>% 
  select("Studie 1", 
         "PatID Studie 1", 
         "timepoints 2022", 
         "Tag PSO", 
...
# serology
sero <- read_excel("../all serology 2021 12 and 10 final_IgG below 7 set 1.xlsx") %>% 
    ...
    print(
      ...
      ggplot(., aes(x = factor(Timepoint, 
        ...
        facet_wrap(~factor(Marker2, levels = c("CD40L+ CD137+",
                   ...
                   scales = "free", ncol = 3) +
        geom_smooth(method = "rlm", fullrange = T, aes(group = factor(Dexa), fill = factor(Dexa)), alpha = 0.1, 
                    se = T, size = 0.4) +
        xlab("Time point") +
  ...
  print(
    ...
    ggplot(aes(x = TagPSO, y = !!sym(i), 
      ...
      facet_wrap(~factor(Marker2, levels = c("CD40L+ CD137+",
                 ...
                 scales = "free", ncol = 3) +
      theme(strip.background =element_rect(fill="aliceblue")) +
      geom_smooth(method = "rlm", fullrange = T, aes(group = factor(Dexa)), alpha = 0.1, 
                  se = F, size = 0.4) +
      xlab("Days PSO") +
      ggtitle(paste0(i, " progression in CD4 cells")) +
...
{  
  print(
    ...
    ggplot(., aes(x = factor(Timepoint, 
      ...
      scale_fill_manual(breaks = c("D+", "D-", "mild"),
                          values = c("orangered", "blue", "green3")) +
      geom_smooth(method = "lm", aes(group = factor(Dexa), fill = factor(Dexa)), alpha = 0.1, 
                  se = F, size = 0.4) + 
...
print(
    PNT %>%
      ...
      #            scales = "free",
      #            labeller = labeller(Dexa = as_labeller(dexa.labels))) +
      geom_smooth(method = "lm", aes(group = factor(Dexa), fill = factor(Dexa)), alpha = 0.2, 
                  se = T, size = 0.4) +
      xlab("Days PSO") +
      ggtitle(paste0(i, " neutralisation progression")) +
      theme_bw() +
...
for(i in SRT)
{
  print(
    Sero_SRT %>% 
      ...
      geom_smooth(method = "lm", se = T))
  ggsave(paste0("MSimages/IgG vs ", i, ".png"), width = 20, height = 10, units = "cm")
}
```

```{r Association of PNT and SRT at timepoints}
PNT_SRT <- PNT %>%
...
ggsave("MSimages/B.1.1.7 vs S1.png", width = 20, height = 10, units = "cm")

# B.1.1.7 vs S2
print(PNT_SRT %>%
    filter(Marker1 == "CD4") %>%
    filter(!(Timepoint == "PI" | Timepoint == "VC")) %>% 
    filter(Marker2 == "CD137+ IFN-g+" | Marker2 == "CD40L+ IL-21+" | Marker2 == "CD137+ TNF-a+" | Marker2 == "CD137+ IL-21+") %>% 
    filter(exclude_B.1.1.7 == 0) %>% 
    as.data.frame() %>%
    ggplot(., aes(x = B.1.1.7, y = S2)) +
      ...
      stat_cor(method = "spearman", size = 4))
ggsave("MSimages/B.1.1.7 vs S2.png", width = 20, height = 10, units = "cm")
...
print(PNT_SRT %>%
    ...
    ggplot(., aes(x = B.1.617.2, y = S1)) +
      ...
      stat_cor(method = "spearman", size = 4))
ggsave(paste0("MSimages/B.1.617.2 vs S1.png"), width = 20, height = 10, units = "cm")
...
print(PNT_SRT %>%
    ...
    ggplot(., aes(x = B.1.617.2, y = S2)) +
      ...
      stat_cor(method = "spearman", size = 4))
ggsave(paste0("MSimages/B.1.617.2 vs S2.png"), width = 10, height = 10, units = "cm")
