...
sero_PNT_cor %>% 
  ...
  xlab("IgG BAU/ml") +
  stat_cor(method = "pearson", label.x = 5000, size = 3) +
...
for(i in SRT)
  print(
    Sero_SRT %>% 
      ...
      ggtitle("IgG vs SRT (CD4)") +
      xlab("IgG BAU/ml") +
      stat_cor(method = "pearson", label.x = 2000, size = 3) +
      geom_smooth(method = "lm", se = F)
  )
...
Sero_meta %>% 
  ...
  geom_violin(alpha = 0.7) +
  geom_jitter(width = 0.1, aes(color = factor(Timepoint))) +
  facet_grid(~factor(Timepoint, level = c('W 2', 'W3-6', 'M2-4', 'after month 5', 'post vacc'))) +
  stat_compare_means(comparisons = my_comparisons) +
  theme_bw() +
  theme(strip.background = element_rect(fill="aliceblue"))
...
Sero_meta %>% 
  ...
  geom_jitter(width = 0.1, aes(color = factor(Timepoint))) +
  facet_grid(~factor(Timepoint, level = c('W 2', 'W3-6', 'M2-4', 'after month 5', 'post vacc'))) +
  stat_compare_means(method = "wilcox.test", comparisons = list(c("0", "3")), label.y = 15000, size = 2.5) +
  stat_compare_means(method = "wilcox.test", comparisons = list(c("0", "1")), label.y = 15400, size = 2.5) +
  stat_compare_means(method = "wilcox.test", comparisons = list(c("1", "3")), label.y = 15900, size = 2.5) +
...
PNT_SRT_diff_timepoints <- PNT %>% 
  left_join(., facs, by = "PatID_Studie1") %>% 
  group_by(PatID_Studie1) %>% 
  #select(PatID_Studie1, Days, Timepoint, Dexa.x) %>% 
...
SRT_W2_PNT_M2 <- PNT_SRT_diff_timepoints %>% 
  filter(PNT_time == "M2-4", SRT_time == "W 2") %>% 
  select(PatID_Studie1, SRT_time, PNT_time) %>% 
  distinct()

SRT_W2_PNT_M5 <- PNT_SRT_diff_timepoints %>% 
  filter(PNT_time == "after month 5", SRT_time == "W 2") %>% 
  select(PatID_Studie1, SRT_time, PNT_time) %>% 
  distinct()

SRT_W3_PNT_M2 <- PNT_SRT_diff_timepoints %>% 
  filter(PNT_time == "M2-4", SRT_time == "W3-6") %>% 
  select(PatID_Studie1, SRT_time, PNT_time) %>% 
  distinct()

SRT_W3_PNT_M5 <- PNT_SRT_diff_timepoints %>% 
...
<!--     facs %>% -->
<!--       filter(Marker1 == "CD4") %>% -->
<!--     ggplot(., aes(x = Timepoint, y = !!sym(i), colour = factor(Dexa))) +  -->
<!--       #geom_point(alpha = 0.5) +  -->
<!--       geom_line(aes(linetype = factor(Dexa), group = factor(Dexa)), alpha = 0.4) + -->
